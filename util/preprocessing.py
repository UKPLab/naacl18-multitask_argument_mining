...
def perpareDataset(embeddingsPath, datasetFiles, frequencyThresholdUnknownTokens=50, reducePretrainedEmbeddings=False, commentSymbol=None):
    ...
    """
    embeddingsPathStrip = os.path.split(embeddingsPath)[1]
    embeddingsName = os.path.splitext(embeddingsPathStrip)[0]
    ...
    for line in embeddingsIn:
        ...
        if (len(split) - 1) > embeddingsDimension:  # Assure that all lines in the embeddings file are of the same length
            print("ERROR: A line in the embeddings file had more dimensions: " + str(len(split) - 1))
            continue
        if (len(split) - 1) < embeddingsDimension:  # Assure that all lines in the embeddings file are of the same length
            print("ERROR: A line in the embeddings file had less dimensions: " + str(len(split) - 1))
            continue
        ...
        if vector.shape[0] != embeddingsDimension:
